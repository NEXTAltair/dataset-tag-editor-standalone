Feature: 画像評価システム
  ...
  Scenario Outline: 複数画像の評価
    ...
    When ユーザーが画像評価機能を起動し、<model_name> を指定して複数の画像を入力する
  ...
  Scenario Outline: 複数モデルでの単一画像の評価
    Given 有効な画像が用意されている
    When ユーザーが画像評価機能を起動し、以下のモデルリストを指定してその単一の画像を入力する:
      | <model1> | <model2> |
    ...
    Examples:
      ...
      | aesthetic_shadow_v1 | ImprovedAesthetic | list        | aesthetic    | [IAP]score_5   |
  ...
  Scenario Outline: 複数モデルでの複数画像の評価
    ...
    When ユーザーが画像評価機能を起動し、以下のモデルリストを指定して複数の画像を入力する:
      | model1   | model2   |
      | <model1> | <model2> |
