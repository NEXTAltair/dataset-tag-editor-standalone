...
└── image_annotator_lib/   # ライブラリパッケージ
    ...
    │   ├── registry.py      # ModelRegistry (モデル登録/取得)
...
### `ModelRegistry` (`core/registry.py`)

- 利用可能なモデルクラスを名前で登録・管理
- `models/`ディレクトリ内のクラスを自動検出
- `list_available_annotators()`関数の基盤
...
1. **初期化とモデル検出**:
   ...
   - `list_available_annotators()`関数が設定ファイル`annotator_config.toml`を読み込み、利用可能なモデルを検出
   - 対応するモデルクラスを`ModelRegistry`から検索

2. **アノテーション実行**:
   - ユーザーが`annotate(images, model_names)`を呼び出し
   - 各モデル名に対して、`ModelRegistry`から対応するクラスを取得しインスタンス化
