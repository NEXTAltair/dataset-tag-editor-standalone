...
*   **`estimated_size_gb` (数値, 推奨)**
    *   モデルのおおよそのサイズをギガバイト (GB) 単位で指定します。
    *   これは `ModelLoad` (`core/model_factory.py`) によるメモリ管理 (キャッシュ制御など) の参考情報として使用されます。
    *   **注意:** モデルが初めてロードされる際に、ライブラリが実際のサイズを計算し、この値を自動的に設定ファイルに書き込む機能があります (`core/utils.py` の `save_model_size`)。手動で設定した値は上書きされる可能性があります。
