...
class DatasetTagEditor(Singleton):
    ...
    def __init__(self):
        ...
        self.re_word = (
            re.compile(settings.current.filename_word_regex)
            if len(settings.current.filename_word_regex) > 0
            else None
        )
            ...
            if backup and txt_path.is_file():
                ...
                if bak_path is None:
                    ...
                    try:
                        ...
                        logger.error(e)
                        logger.write(f"A backup file of {txt_path} cannot be created.")
                    else:
                        backup_num += 1
            # save
            try:
        ...
        for data in self.dataset.datas.values():
            for tag in data.tags:
                ...
                if tag not in self.tag_tokens:
                    self.tag_tokens[tag] = clip_tokenizer.tokenize(tag)
