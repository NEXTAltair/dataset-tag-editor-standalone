...
def gather_available_classes(directory: str) -> dict[str, ModelClass]:
    ...
    for module_file in module_files:
        ...
        for name, obj in inspect.getmembers(module, inspect.isclass):
            # BaseTagger のサブクラスか、または predict メソッドを持つ場合､独自色の強いBLIP用
            if (issubclass(obj, BaseTagger) and obj is not BaseTagger) or hasattr(obj, "predict"):
                available[name] = obj
...
def register_taggers() -> dict[str, ModelClass]:
    """利用可能なスコアラークラスを登録"""
    # 開始ログ
    caller = traceback.extract_stack()[-2]
    logger.debug(f"register_taggers開始: 呼び出し元={caller[0]}:{caller[1]} in {caller[2]}")
...
def list_available_taggers() -> list[str]:
    ...
    Example:
        >>> from tagger_wrapper_lib import list_available_taggers
        >>> models = list_available_taggers()
        >>> print(models)
...
# モジュールロード時の初期化ログ
logger.debug("モジュール初期化: registry.py がロードされました")
register_taggers()
